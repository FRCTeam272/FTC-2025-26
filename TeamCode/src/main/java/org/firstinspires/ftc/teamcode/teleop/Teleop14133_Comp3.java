...
@TeleOp (name="Teleop14133_Comp3", group = "Competition")
public class Teleop14133_Comp3 extends SampleCommandTeleop {
...
//        telemetry.addData("Vision Range to Goal", vision.getTagRange());
    ...
    public void runToggledDrive() {
        ...
        if (lockPressed && !lockPrevPressed) {
            if (drive.getDriveMode() == DriveSubsystemV3.DriveMode.MANUAL) {
                drive.setDriveMode(DriveSubsystemV3.DriveMode.LOCKED_ON_GOAL);
            } else {
                drive.setDriveMode(DriveSubsystemV3.DriveMode.MANUAL);
            }
